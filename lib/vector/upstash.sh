...
ingo_vector_upstash_base_url() {
  printf "%s" "${INGO_VECTOR_URL:-${UPSTASH_VECTOR_REST_URL:-}}"
}

ingo_vector_upstash_token() {
  printf "%s" "${INGO_VECTOR_TOKEN:-${UPSTASH_VECTOR_REST_TOKEN:-}}"
}

ingo_vector_upstash_require_env() {
  local missing=0

  if [ -z "$(ingo_vector_upstash_base_url)" ]; then
    echo "missing env var: INGO_VECTOR_URL (legacy: UPSTASH_VECTOR_REST_URL)" >&2
    missing=1
  fi
  if [ -z "$(ingo_vector_upstash_token)" ]; then
...
ingo_vector_upstash_doctor() {
  ingo_vector_upstash_require_env || return $?
  printf "vector: backend=upstash namespace=%s url=%s auth=token\n" \
    "$INGO_NAMESPACE" \
    "$(ingo_vector_upstash_base_url)"
}

ingo_vector_upstash_build_upsert_payload() {
  local line="$1"
  local namespace="$2"
  local meta_file="${3:-}"
...
ingo_vector_upstash_upsert_line_impl() {
  ...
  ingo_vector_upstash_require_env || return $?
  payload="$(ingo_vector_upstash_build_upsert_payload "$line" "$namespace" "$meta_file")"

  response="$(ingo_http_curl -sS -w "\n%{http_code}" \
    -X POST "$(ingo_vector_upstash_base_url)/upsert-data" \
    -H "Authorization: Bearer $(ingo_vector_upstash_token)" \
...
ingo_vector_upstash_query_text() {
  local question="$1"
  local top_k="$2"
  local namespace="$3"
  local payload response status body
  ...
  response="$(ingo_http_curl -sS -w "\n%{http_code}" \
    -X POST "$(ingo_vector_upstash_base_url)/query-data" \
    -H "Authorization: Bearer $(ingo_vector_upstash_token)" \
    -H "Content-Type: application/json" \
    -d "$payload")"
